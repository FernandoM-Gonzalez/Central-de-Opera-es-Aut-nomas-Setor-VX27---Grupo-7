...
struct Operador {
    ...
    char nomeOperador[70], setorOperador[10], nivelOperacional[30], statusOperador[20];
};

struct Equipamento {
    int idOperadorEquipamento, nivelPrioridade;
    char idEquipamento[10], tipoEquipamento[20], setorEquipamento[10], estadoOperacional[20];
};
...
void cadastroOperador(struct Informacoes *info) {
    ...
    char leitorStr[50], nomeOperadorTemporario[70], setorOperadorTemporario[10], nivelOperacionalTemporario[30], statusOperadorTemporario[30];
    ...
    do {
        ...
        fgets(setorOperadorTemporario, sizeof(setorOperadorTemporario), stdin);
...
int verificadorSetorExistente(char setor[], struct Informacoes *info) {
    for (int i = 0; i < TAM_LISTA_OPERADORES; i++) {
        // Ignora slots de operadores não cadastrados
        if (info->listaOperadores[i].idOperador != 0 &&
            strcasecmp(setor, info->listaOperadores[i].setorOperador) == 0) {
            return 1;
        }
    }
    return 0;
...
int verificadorIdExistente(int id, struct Informacoes *info) {
    for (int i = 0; i < TAM_LISTA_OPERADORES; i++) {
        if (info->listaOperadores[i].idOperador != 0 &&
            id == info->listaOperadores[i].idOperador) {
            return 1;
        }
    }
    return 0;
...
void cadastroEquipamento(struct Informacoes *info) {
    int verificador = 0, leitorNum = 0, nivelPrioridadeTemporario = 0, idOperadorEquipamentoTemporario = 0;
    ...
    do {
        ...
        fgets(setorEquipamentoTemporario, sizeof(setorEquipamentoTemporario), stdin);
...
void atualizarDisponibilidadeOperador(struct Informacoes *bancoDeDados) {
    ...
    for (int i = 0; i < TAM_LISTA_OPERADORES; i++) {
        if (bancoDeDados->listaOperadores[i].idOperador == idInformado) {
            do {
                printf("[ 1 - Disponível | 2 - Ocupado | 3 - Inativo | 4 - Bloqueado ]\n Status do Operador: ");
                int novoStatus;
                scanf("%d", &novoStatus);
                switch (novoStatus) {
                    case 1:
                        strcpy(bancoDeDados->listaOperadores[i].statusOperador, "Disponivel");
                        verificador = 1;
                        break;
                    case 2:
                        strcpy(bancoDeDados->listaOperadores[i].statusOperador, "Ocupado");
                        verificador = 1;
                        break;
                    case 3:
                        strcpy(bancoDeDados->listaOperadores[i].statusOperador, "Inativo");
                        verificador = 1;
                        break;
                    case 4:
                        strcpy(bancoDeDados->listaOperadores[i].statusOperador, "Bloqueado");
                        verificador = 1;
                        break;
                    default:
                        printf("Opcão Inválida. Tente novamente.\n\n");
            ...
            break; // Operador encontrado e atualizado, sai do laço
        }
    }
}
...
void criarRegistro(struct Informacoes *bancoDeDados) {
    ...
    for (int i = 0; i < TAM_LISTA_EQUIPAMENTOS; i++) {
        if (bancoDeDados->registrosGerais[i].id == 0) {
            bancoDeDados->registrosGerais[i].id = bancoDeDados->qtdRegistrosCriados + 1;
...
void atualizar(struct Informacoes *bancoDeDados) {
    ...
    for (;;) {
        ...
        do {
            ...
            a = scanf("%d", &op);
            limpaBuffer();
        } while (a != 1);   // repetir enquanto não conseguiu ler um inteiro
...
int consulta_id(char id[], struct Informacoes *BancoDeDados) {
    int idConvertido = atoi(id);

    for (int i = 0; i < TAM_LISTA_OPERADORES; i++) {
        if (idConvertido == BancoDeDados->listaOperadores[i].idOperador) {
            if (strcmp("Disponivel", BancoDeDados->listaOperadores[i].statusOperador) == 0) {
                printf("O Operador de ID consultado esta livre!\n");
                return i;
            }

            printf("O Operador de ID consultado esta ocupado!\n");
            return -1;
        }
    }

    printf("Operador com o ID informado nao foi encontrado.\n");
    return -2;
}

int consulta_nomeLivre(char nome[], struct Informacoes *BancoDeDados) {
    for (int i = 0; i < TAM_LISTA_OPERADORES; i++) {
        if (strcmp(nome, BancoDeDados->listaOperadores[i].nomeOperador) == 0) {
            if (strcmp("Disponivel", BancoDeDados->listaOperadores[i].statusOperador) == 0) {
                printf("O Operador de nome consultado esta livre!\n");
                return i;
            }

            printf("O Operador de nome consultado esta ocupado!\n");
            return -1;
        }
    }

    printf("Operador com o nome informado nao foi encontrado.\n");
    return -2;
}

void consulta_estadoOPERACIONAL(char id[], struct Informacoes *bancoDeDados){
    for (int i = 0; i < TAM_LISTA_EQUIPAMENTOS; i++) {
        if (strcmp(id, bancoDeDados->listaEquipamentos[i].idEquipamento) == 0){
            if (strcmp("Ativo", bancoDeDados->listaEquipamentos[i].estadoOperacional) == 0){

                printf("o equipamento está ativo!");
  }
   else if (strcmp("Inativo", bancoDeDados->listaEquipamentos[i].estadoOperacional) == 0){
                printf("o equipamento está inativo!");
            }


 else if (strcmp("Manutencao", bancoDeDados->listaEquipamentos[i].estadoOperacional) == 0){
                printf("O equipamento está em manutenção!\n");
...
void consultar_operadores_livres_por_equipamento_setor(char equipamento[], char setor[], struct Informacoes *BancoDeDados) {
    int encontrouQualquer = 0;

    printf("\n--- Operadores Livres para %s no setor %s ---\n", equipamento, setor);

    for (int i = 0; i < TAM_LISTA_OPERADORES; i++) {
        if (BancoDeDados->listaOperadores[i].idOperador != 0 &&
            strcmp("Disponivel", BancoDeDados->listaOperadores[i].statusOperador) == 0 &&
            strcmp(setor, BancoDeDados->listaOperadores[i].setorOperador) == 0) {
    ...
    if (!encontrouQualquer) {
        printf("Nenhum operador livre atende a estes criterios no momento.\n");
    }
}
