...
struct Operador {
    int idOperador;
    int qtdOperacoes;
    char nomeOperador[TAM_NOME];
    char setorOperador[TAM_SETOR];
    char nivelOperacional[30];
    char statusOperador[30];
};

struct Equipamento {
    int idOperadorEquipamento;
    int nivelPrioridade;
    char idEquipamento[10];
    char tipoEquipamento[30];
    char setorEquipamento[TAM_SETOR];
    char estadoOperacional[30];
};
...
struct Informacoes {
    ...
    int qtdEquipamentosCadastrados;
...
int verificadorPadraoSetor(char setor[]) {
    ...
    if (!isalpha((unsigned char) setor[0]) || !isalpha((unsigned char) setor[1])) {
        return 0;
    }

    if (!isdigit((unsigned char) setor[2]) || !isdigit((unsigned char) setor[3])) {
        return 0;
    }
...
int verificadorPadraoIdEquipamento(char id[]) {
    ...
    if (!isdigit((unsigned char) id[1]) || !isdigit((unsigned char) id[2])) {
        return 0;
    }
...
int buscarIndiceOperadorPorId(struct Informacoes *info, int id) {
    ...
    for (i = 0; i < info->qtdOperadoresCadastrados; i++) {
        if (info->listaOperadores[i].idOperador == id) {
            return i;
        }
    }
...
int buscarIndiceOperadorPorNome(struct Informacoes *info, char nome[]) {
    ...
    for (i = 0; i < info->qtdOperadoresCadastrados; i++) {
        if (compararTexto(info->listaOperadores[i].nomeOperador, nome) == 1) {
            return i;
        }
    }
...
int buscarIndiceEquipamento(struct Informacoes *info, char id[]) {
    ...
    for (i = 0; i < info->qtdEquipamentosCadastrados; i++) {
        if (compararTexto(info->listaEquipamentos[i].idEquipamento, id) == 1) {
            return i;
        }
    }
...
int buscarIndiceRegistro(struct Informacoes *info, int id) {
    ...
    for (i = 0; i < info->qtdRegistrosCriados; i++) {
        if (info->registrosGerais[i].id == id) {
            return i;
        }
    }
...
int verificadorNomeRepetido(char nome[], struct Informacoes *info) {
    ...
    if (buscarIndiceOperadorPorNome(info, nome) != -1) {
        return 1;
    }
...
int verificadorSetorExistente(char setor[], struct Informacoes *info) {
    ...
    for (i = 0; i < info->qtdOperadoresCadastrados; i++) {
        if (compararTexto(setor, info->listaOperadores[i].setorOperador) == 1) {
            return 1;
        }
    }
...
int verificadorIdExistente(int id, struct Informacoes *info) {
    if (buscarIndiceOperadorPorId(info, id) != -1) {
        return 1;
    }
...
int operadorPodeControlar(struct Operador op) {
    if (compararTexto(op.statusOperador, "Inativo") == 1) {
        return 0;
    }

    if (compararTexto(op.statusOperador, "Bloqueado") == 1) {
        return 0;
    }
...
int operadorEstaLivre(struct Operador op) {
    if (compararTexto(op.statusOperador, "Disponivel") == 1) {
        return 1;
    }
...
void cadastroOperador(struct Informacoes *info) {
    int verificador;
    char leitorStr[50];
    char nomeOperadorTemporario[TAM_NOME];
    char setorOperadorTemporario[TAM_SETOR];
    char nivelOperacionalTemporario[30];
    char statusOperadorTemporario[30];
...
void cadastroEquipamento(struct Informacoes *info) {
    int verificador;
    char idEquipamentoTemporario[10];
    char tipoEquipamentoTemporario[30];
    char setorEquipamentoTemporario[TAM_SETOR];
    char estadoOperacionalTemporario[30];
        ...
        if (compararTexto(bancoDeDados->listaEquipamentos[i].setorEquipamento, setor) == 1) {
            ...
            if (indiceOperador == -1) {
                ...
                printf("| %04d        | %-28s | %-14s | %-20s |\n",
                       bancoDeDados->listaOperadores[indiceOperador].idOperador,
                       bancoDeDados->listaOperadores[indiceOperador].nomeOperador,
                       bancoDeDados->listaEquipamentos[i].idEquipamento,
                       bancoDeDados->listaEquipamentos[i].tipoEquipamento);
            }
...
int setorJaFoiMostrado(char setores[][TAM_SETOR], int qtdSetores, char setor[]) {
    ...
    for (i = 0; i < qtdSetores; i++) {
        if (compararTexto(setores[i], setor) == 1) {
            return 1;
        }
    }
...
int pesoNivelOperador(struct Operador op) {
    if (compararTexto(op.nivelOperacional, "Supervisor Tecnico") == 1) {
        return 3;
    }

    if (compararTexto(op.nivelOperacional, "Intermediario") == 1) {
        return 2;
    }
...
int escolherEquipamentoBaixaParaRealocacao(struct Informacoes *bancoDeDados, int indiceEquipamentoAlta) {
    int i;
    int indiceMelhor = -1;
    int indiceOperadorAtual;
    int indiceOperadorMelhor;
    int idOperadorAlta;
    int pesoAtual;
    int pesoMelhor;
    ...
    for (i = 0; i < bancoDeDados->qtdEquipamentosCadastrados; i++) {
        if (i == indiceEquipamentoAlta) {
            continue;
        }

        if (bancoDeDados->listaEquipamentos[i].nivelPrioridade != 1) {
            continue;
        }

        if (compararTexto(bancoDeDados->listaEquipamentos[i].setorEquipamento,
                          bancoDeDados->listaEquipamentos[indiceEquipamentoAlta].setorEquipamento) == 0) {
            continue;
        }

        if (bancoDeDados->listaEquipamentos[i].idOperadorEquipamento == 0) {
            continue;
        }

        if (bancoDeDados->listaEquipamentos[i].idOperadorEquipamento == idOperadorAlta) {
            continue;
        }
        ...
        if (indiceOperadorAtual == -1) {
            continue;
        }

        if (operadorPodeControlar(bancoDeDados->listaOperadores[indiceOperadorAtual]) == 0) {
            continue;
        }
...
void trocarOperadorAutomaticoPrioridadeAlta(struct Informacoes *bancoDeDados) {
    char id[10];
    int indiceEquipamentoAlta;
    int indiceEquipamentoBaixa;
    int idOperadorSaiuDaBaixa;
    int idOperadorAntigoDaAlta;
